...
function App() {
  ...
  const unlock = async (password: string) => {
    try {
      ...
      await provider.connect(googleProvider && !googleProvider.isConnected() ? { loginHint: metadata.googleAccountEmail } : undefined)
  ...
  if (locked || !vault) {
    return <UnlockScreen username={preferences.username} accountEmail={preferences.driveMetadata?.googleAccountEmail} onUnlock={unlock} onSwitchGoogleAccount={preferences.storageMode === 'google' ? switchGoogleAccount : undefined} notice={notice} />
  }
...
function UnlockScreen({ username, accountEmail, onUnlock, onSwitchGoogleAccount, notice }: { username: string; accountEmail?: string; onUnlock: (password: string) => Promise<void>; onSwitchGoogleAccount?: () => Promise<void>; notice: Notice | null }) {
  const [password, setPassword] = useState(''); const [busy, setBusy] = useState(false); const [switching, setSwitching] = useState(false); const [error, setError] = useState('')
  ...
  const switchAccount = async () => { if (!onSwitchGoogleAccount) return; setSwitching(true); setError(''); try { await onSwitchGoogleAccount() } catch (switchError) { setError(switchError instanceof Error ? switchError.message : 'Could not switch Google account.') } finally { setSwitching(false) } }
